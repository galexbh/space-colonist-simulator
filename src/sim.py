...
class Simulador:
    ...
    def __init__(self, poblacion_inicial, recursos_iniciales, num_anios=365):
        ...
        self.recuperados = []
        self.muertes = []
        self.eventos_catastroficos = [
            {"nombre": "Exposición a la radiación",
                "muertes_aprox": 0.3, "probabilidad": 0.3},
            {"nombre": "Conflictos internos",
                "muertes_aprox": 0.02, "probabilidad": 0.5},
            {"nombre": "Lluvia de Metoritos",
                "muertes_aprox": 0.1, "probabilidad": 0.2}
        ]
    ...
    def calcular_muertes_por_recursos(self, poblacion_actual, recursos_disponibles, consumo_recursos_por_persona):
        ...
        if recursos_disponibles == 0:
            ...
            muertes_por_recursos = round(
                poblacion_actual) - (recursos_disponibles / consumo_recursos_por_persona)
            recursos_disponibles = recursos_disponibles - \
                recursos_disponibles * consumo_recursos_por_persona
    ...
    def ajustar_recursos(self, poblacion_actual, recursos_disponibles, consumo_recursos_por_persona, muertes_por_recursos):
        ...
        if recursos_disponibles <= 0:
            ...
            recursos_disponibles -= round(poblacion_actual) * \
                consumo_recursos_por_persona
    ...
    def simular_evento_catastrofico(self, eventos_catastroficos, poblacion_actual):
        ...
        for evento in eventos_catastroficos:
            if np.random.rand() < evento["probabilidad"]:
                evento_muertes1 = evento_muertes1 + \
                    round(evento["muertes_aprox"]*poblacion_actual)
    ...
    def simular_modelo_sir_mortalidad(self, poblacion_actual, proporcion_individuos_infectados, beta, gamma, tasa_mortalidad_enfermedad, anio):
        ...
        self.susceptibles.append(
            round(poblacion_actual*(1-proporcion_individuos_infectados)))
        self.infectados.append(
            round(poblacion_actual*(proporcion_individuos_infectados)))

        nuevas_infecciones = round(
            beta * self.susceptibles[-1] * self.infectados[-1] / poblacion_actual)

        nuevas_recuperaciones = int(gamma * self.infectados[-1])
        nuevas_muertes = int(tasa_mortalidad_enfermedad * self.infectados[-1])

        self.susceptibles.append(
            self.susceptibles[anio-1] - nuevas_infecciones)
        self.infectados.append(
            self.infectados[anio-1] + nuevas_infecciones - nuevas_recuperaciones - nuevas_muertes)
    ...
    def simular(self, tasa_natalidad, tasa_mortalidad, consumo_recursos_por_persona, recursos_por_expedicion, probabilidad_exito_expedicion, proporcion_individuos_infectados, beta, gamma, tasa_mortalidad_enfermedad):
        for anio in range(self.num_anios):
            ...
            if self.poblacion_actual > 0:
                ...
                muertes_por_recursos = self.calcular_muertes_por_recursos(
                    self.poblacion_actual, self.recursos_disponibles, consumo_recursos_por_persona)

                # Ajustar los recursos de la poblacion
                self.recursos_disponibles = self.ajustar_recursos(
                    self.poblacion_actual, self.recursos_disponibles, consumo_recursos_por_persona, muertes_por_recursos)
                ...
                muertes_por_edad = round(
                    self.poblacion_actual * tasa_mortalidad)
            ...
            if self.poblacion_actual > 0:
                total_muertes = self.simular_modelo_sir_mortalidad(
                    self.poblacion_actual, proporcion_individuos_infectados, beta, gamma, tasa_mortalidad_enfermedad, anio)
                self.poblacion_actual -= total_muertes

                # probabilidad de evento catastrofico
            if self.poblacion_actual > 0:
                evento_muertes = self.simular_evento_catastrofico(
                    self.eventos_catastroficos, self.poblacion_actual)
